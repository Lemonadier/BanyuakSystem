...
const uiModule = {
    ...
    renderStudentCards: () => {
        ...
        container.innerHTML = studentsToDisplay.map(s => {
            ...
            const tx = state.transactions.filter(t => String(t['Student ID']) === String(s['Student ID']));
            let recentHtml = '';
            if (sys === 'bank') {
                const deposits = tx.filter(t => t.Type === 'Deposit').reduce((sum, t) => sum + (parseFloat(t.Amount) || 0), 0);
                const withdraws = tx.filter(t => t.Type === 'Withdraw').reduce((sum, t) => sum + (parseFloat(t.Amount) || 0), 0);
                recentHtml = `<div class="text-xs mt-1 flex gap-2">
                    <span class="text-emerald-600 font-semibold">+฿${app.formatMoney(deposits)}</span>
                    <span class="text-red-600 font-semibold">-฿${app.formatMoney(withdraws)}</span>
                </div>`;
            } else if (sys === 'attendance') {
                const lastTx = tx.sort((a, b) => new Date(b.Date) - new Date(a.Date))[0];
                recentHtml = `<div class="text-xs text-slate-400 mt-1">Last: ${lastTx ? (lastTx.Status || lastTx.Type) : 'N/A'}</div>`;
            } else if (sys === 'health') {
                const lastTx = tx.sort((a, b) => new Date(b.Date) - new Date(a.Date))[0];
                recentHtml = `<div class="text-xs text-slate-400 mt-1">${lastTx ? `W: ${lastTx.Weight}kg / H: ${lastTx.Height}cm` : 'No data'}</div>`;
            }

            return `<div class="bg-white p-5 rounded-xl border border-slate-200 hover:shadow-md transition-all cursor-pointer" onclick="app.showStudentDetail('${s['Student ID']}')">
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-12 h-12 rounded-full ${SYSTEMS[sys].color} text-white flex items-center justify-center font-bold text-lg">${s.No || s.Name?.charAt(0) || '?'}</div>
                    <div class="flex-1">
                        <div class="font-bold text-slate-800">${s.Name}</div>
                        <div class="text-xs text-slate-400">ID: ${s['Student ID']}</div>
                        ${recentHtml}
                    </div>
                </div>
                ${dataDisplay}
                ...
                if (matchesSearch && matchesDateFrom && matchesDateTo) {
                    if (CONFIG.role === 'teacher' || String(s['Student ID']) === String(CONFIG.user?.['Student ID'])) {
                        ...
                        if (CONFIG.activeSystem === 'bank') {
                            ...
                            const typeColor = item.Type === 'Deposit' ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50';
                            const amountColor = item.Type === 'Deposit' ? 'text-emerald-700' : 'text-red-700';
                            const amountSign = item.Type === 'Deposit' ? '+' : '-';
                            tbody.innerHTML += `<tr class="border-b border-slate-100 hover:bg-slate-50">
                                <td class="px-4 py-3 text-sm"><b>${s.Name}</b><br><span class="text-gray-400">${dateStr}</span></td>
                                <td class="px-4 py-3 text-sm">
                                    <span class="px-2 py-1 rounded-md ${typeColor} font-semibold text-xs">${item.Type}</span>
                                </td>
                                <td class="px-4 py-3 text-sm font-mono font-bold ${amountColor}">${amountSign}฿${app.formatMoney(item.Amount)}</td>
                                <td class="px-4 py-3 text-xs text-slate-400">${item.Note || '-'}</td>
                            </tr>`;
                        } else if (CONFIG.activeSystem === 'attendance') {
                            typeDisplay = item.Status || 'Unknown';
                            valueDisplay = '';
                            tbody.innerHTML += `<tr class="border-b border-slate-100 hover:bg-slate-50">
                                <td class="p-3 text-xs"><b>${s.Name}</b><br><span class="text-gray-400">${typeDisplay} (${dateStr})</span></td>
                                <td class="p-3 text-right font-mono">${valueDisplay}</td>
                            </tr>`;
                        } else if (CONFIG.activeSystem === 'health') {
    ...
    openModal: (id) => {
        ...
        if (modal) {
            ...
            if (id === 'transaction') {
                ...
                // Always load current API URL from config or localStorage
                const apiUrl = CONFIG.apiUrl || localStorage.getItem('cb_api_url') || localStorage.getItem('cb_session_api_url') || localStorage.getItem('cb_teacher_api_key') || '';
                const apiInput = document.getElementById('api-url');
                if (apiInput) {
                    apiInput.value = apiUrl;
                    apiInput.removeAttribute('readonly');
                    apiInput.removeAttribute('disabled');
                    apiInput.focus(); // Focus to help user paste immediately
                }
            }
        }
    },
    ...
    switchAuthTab: (role) => {
        ...
        if (role === 'student') {
            studentTab.className = 'flex-1 py-2 rounded-lg text-sm font-semibold transition-all bg-white shadow-sm text-indigo-600';
            teacherTab.className = 'flex-1 py-2 rounded-lg text-sm font-semibold text-slate-500 hover:text-indigo-500 transition-all';
            studentForm.classList.remove('hidden');
            teacherForm.classList.add('hidden');
        } else {
            teacherTab.className = 'flex-1 py-2 rounded-lg text-sm font-semibold transition-all bg-white shadow-sm text-indigo-600';
            studentTab.className = 'flex-1 py-2 rounded-lg text-sm font-semibold text-slate-500 hover:text-indigo-500 transition-all';
            teacherForm.classList.remove('hidden');
            studentForm.classList.add('hidden');

            if (CONFIG.isSetupMode) document.getElementById('msg-setup-required').classList.remove('hidden');
        }
    },

    switchSettingTab: (tab) => {
        const conTab = document.getElementById('set-tab-connection');
        const secTab = document.getElementById('set-tab-security');
        const conContent = document.getElementById('set-content-connection');
        const secContent = document.getElementById('set-content-security');

        if (tab === 'connection') {
            conTab.className = 'flex-1 pb-2 border-b-2 border-indigo-600 text-indigo-600 font-bold text-sm';
            secTab.className = 'flex-1 pb-2 border-b-2 border-transparent text-slate-500 font-bold text-sm hover:text-slate-700';
            conContent.classList.remove('hidden');
            secContent.classList.add('hidden');
        } else {
            secTab.className = 'flex-1 pb-2 border-b-2 border-indigo-600 text-indigo-600 font-bold text-sm';
            conTab.className = 'flex-1 pb-2 border-b-2 border-transparent text-slate-500 font-bold text-sm hover:text-slate-700';
            secContent.classList.remove('hidden');
            conContent.classList.add('hidden');
        }
    },
...
window.switchAuthTab = uiModule.switchAuthTab;
window.switchSettingTab = uiModule.switchSettingTab;
window.openWeeklyFullscreen = uiModule.toggleWeeklyFullscreen;
