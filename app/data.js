...
const dataModule = {
    ...
    postData: async (payload, msg) => {
        app.loading(true);
        try {
            // CRITICAL: Always include admin key for authorization
            const adminKey = localStorage.getItem('cb_session_token') || '';
            const reqPayload = { ...payload, adminKey };
            ...
            const data = await res.json();
            if (data.status === 'success') {
                await app.fetchData(null); // Refetch data
