...
const app = {
    ...
    init: () => {
        ...
        if (localStorage.getItem('cb_magic_child_id') && !CONFIG.role) {
            ...
            // Ensure API URL is loaded and activeSystem is set before login attempt
            const apiUrl = localStorage.getItem('cb_api_url') || localStorage.getItem('cb_session_api_url');
            if (apiUrl) {
                CONFIG.apiUrl = apiUrl;
                CONFIG.activeSystem = 'bank'; // Set default system for magic link login
            }

            setTimeout(() => {
                authModule.handleStudentLogin({ preventDefault: () => { } });
                localStorage.removeItem('cb_magic_child_id');
            }, 500); // Increased timeout to ensure API is ready
        }
    ...
    setTimeGranularity: (gran) => {
        state.timeGranularity = gran;
        // Update button styles
        document.querySelectorAll('.time-gran-btn').forEach(btn => {
            btn.classList.remove('bg-white', 'text-indigo-600');
            btn.classList.add('text-slate-600');
        });
        const activeBtn = document.getElementById(`btn-time-${gran}`);
        if (activeBtn) {
