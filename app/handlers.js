...
const handlersModule = {
    ...
    populateMultiTxFields: () => {
        ...
        // Update the student list container
        const listContainer = document.getElementById('multi-student-list');
        if (listContainer) listContainer.innerHTML = studentHtml;

        // Update the fields container
        const fieldContainer = document.getElementById('dynamic-multi-fields');
        if (fieldContainer) fieldContainer.innerHTML = fieldHtml;
    },
    ...
    saveApiUrl: () => {
        ...
        if (url) {
            ...
            CONFIG.apiUrl = url; // Update memory immediately
