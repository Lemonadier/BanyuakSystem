...
const chartsModule = {
    renderCharts: () => {
        ...
        if (CONFIG.activeSystem === 'attendance' && (chartType === 'frequency' || chartType === 'rate')) {
            ...
            // Check if time-series view is active
            const timeGran = state.timeGranularity || 'day';
            if (timeGran !== 'student' && (CONFIG.activeSystem === 'bank' || CONFIG.activeSystem !== 'attendance')) {
                // Time-based grouping for non-attendance systems or when explicitly requested
                const timeGroups = {};
                ...
                filteredTxns.forEach(t => {
                    ...
                    if (timeGran === 'hour') {
                        key = `${txDate.getFullYear()}-${String(txDate.getMonth() + 1).padStart(2, '0')}-${String(txDate.getDate()).padStart(2, '0')} ${String(txDate.getHours()).padStart(2, '0')}:00`;
                    } else if (timeGran === 'day') {
                        key = `${txDate.getFullYear()}-${String(txDate.getMonth() + 1).padStart(2, '0')}-${String(txDate.getDate()).padStart(2, '0')}`;
                    ...
                    if (!timeGroups[key]) {
                        timeGroups[key] = { deposits: 0, withdrawals: 0, total: 0, count: 0 };
                    }

                    const val = parseFloat(t.Amount) || parseFloat(t.Score) || parseFloat(t.Weight) || 0;
                    timeGroups[key].count++;

                    if (CONFIG.activeSystem === 'bank') {
                        if (t.Type === 'Deposit') {
                            timeGroups[key].deposits += val;
                            timeGroups[key].total += val;
                        } else {
                            timeGroups[key].withdrawals += val;
                            timeGroups[key].total -= val;
                        }
                    } else {
                        timeGroups[key].total += val;
                ...
                if (CONFIG.activeSystem === 'bank') {
                    // Show deposits and withdrawals separately for bank system
                    datasets = [
                        {
                            label: 'Deposits',
                            data: sortedKeys.map(k => timeGroups[k].deposits),
                        ...
                        {
                            label: 'Withdrawals',
                            data: sortedKeys.map(k => timeGroups[k].withdrawals),
                    ...
                    datasets = [{
                        ...
                        data: sortedKeys.map(k => timeGroups[k].total / (timeGroups[k].count || 1)),
