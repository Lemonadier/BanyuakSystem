    ...
    checkSetupStatus: async () => {
        try {
            ...
            if (data.status === 'success' && data.setup_required) {
                ...
                document.getElementById('btn-login-action').textContent = 'Create Admin Key';
                document.querySelector('label[data-i18n="label_access_pin"]').textContent = 'Create New Admin Key';
                document.getElementById('login-teacher-key').placeholder = 'Create your secure key';
                window.switchAuthTab('teacher');
                Swal.fire({
                    icon: 'success',
                    title: 'Connected!',
                    text: 'API connected. Please create your Admin Key.',
                    timer: 2500,
                    showConfirmButton: false
                });
            } else if (data.status === 'success') {
                CONFIG.isSetupMode = false;
                // Reset UI for Login Mode
                document.getElementById('msg-setup-required').classList.add('hidden');
                document.getElementById('btn-login-action').textContent = 'Login';
                document.querySelector('label[data-i18n="label_access_pin"]').textContent = 'Admin Key';
                document.getElementById('login-teacher-key').placeholder = 'Enter Admin Key';
                Swal.fire({
                    icon: 'success',
                    title: 'Connected!',
                    text: 'API connected successfully. You can now login.',
                    timer: 2500,
                    showConfirmButton: false
                });
            }
            ...
            if (e.name === 'AbortError') {
                Swal.fire('Connection Error', 'Could not connect to API (Timeout). Check your URL.', 'error');
            }
